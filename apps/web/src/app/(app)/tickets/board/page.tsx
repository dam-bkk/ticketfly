...
const COLUMNS = ["open", "in_progress", "pending", "on_hold", "resolved"] as const;
  ...
  return (
    <>
      ...
      <div className="flex h-10 shrink-0 items-center gap-1 bg-surface px-3 hairline-b">
        ...
        <span className="ml-3 text-[12px] text-ink-3">Grouped by status · drag-and-drop lands with the Projects module</span>
      </div>
                  ...
                  {cards.map((t) => (
                    <Link key={t.id} href={`/tickets/${t.id}`} className={cn("block rounded-lg bg-surface p-3 transition-shadow hairline hover:shadow-2")}>
                      <div className="flex items-start gap-2">
                        <PriorityMark priority={t.priority} className="mt-0.5" />
                        <span className="min-w-0 flex-1 text-[13px] font-medium leading-snug">{t.subject}</span>
                      </div>
                      <div className="mt-2 flex items-center gap-2 text-[11.5px] text-ink-3">
                        <span className="font-mono">{t.legacyRef ?? formatTicketRef(t.id)}</span>
                        <span className="truncate">{t.requester}</span>
                        <span className="ml-auto flex items-center gap-1.5">
                          {status !== "resolved" && <SlaChip sla={t.sla} compact />}
                          {t.assignee ? <Avatar name={t.assignee} size={18} /> : <span className="size-[18px] rounded-full border border-dashed border-line-strong" />}
                        </span>
                      </div>
                      <p className="mt-1 text-[11px] text-ink-4">{relTime(t.updatedAt)}</p>
                    </Link>
                  ))}
