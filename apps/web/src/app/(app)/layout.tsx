...
import { inboxCounts, taskCounts } from "@/lib/queries";
...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const [extra] = (await db.execute(sql`select
    ...
    (select count(*) from alerts where status = 'new')::int as alerts`)) as unknown as { approvals: number; alerts: number }[];
  const tc = await taskCounts(me.id);
  const prefs = await getPrefs(me.id);
  const va = await viewAsContext(me);
  return (
    <div className="flex h-dvh overflow-hidden">
      <Sidebar me={me} counts={{ open: c.open, mine: c.mine, atRisk: c.atRisk, onboarding: ob?.n ?? 0, approvals: extra?.approvals, tasks: tc.mine, alerts: extra?.alerts }} version={APP_VERSION} hidden={prefs.hiddenModules} />
