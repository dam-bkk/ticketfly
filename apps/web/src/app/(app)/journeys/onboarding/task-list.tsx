    ...
    <div aria-busy={pending}>
      ...
      <ul className={cn("space-y-0.5", compact && "space-y-0")}>
        {tasks.map((t) => {
          ...
          return (
            <li key={t.key}>
              <button onClick={() => start(() => toggleOnboardingTask(onboardingId, t.key))} className={cn("group flex w-full items-center gap-3 rounded-md px-2 py-1.5 text-left text-[13px] transition-colors hover:bg-surface-2", compact && "py-1")}>
                ...
                <span className={cn("tnum w-16 text-right text-[11.5px]", overdue ? "font-medium text-crit" : "text-ink-4")}>{t.dueOffsetDays === 0 ? "Day 1" : `${t.dueOffsetDays > 0 ? "+" : ""}${t.dueOffsetDays}d`}</span>
                {!compact && <span className="tnum w-14 text-right text-[11.5px] text-ink-4">{format(due, "d MMM")}</span>}
              </button>
            </li>
          );
        })}
