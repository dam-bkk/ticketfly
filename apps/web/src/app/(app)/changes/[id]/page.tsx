...
import { decideApproval, transitionChange } from "@/app/module-actions";
...
export default async function ChangePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const next =
    ...
    : c.status === "awaiting_approval" ? (myApproval?.decision === "pending" ? { label: "your approval", hint: "Read the plan and rollback below, then decide." } : { label: "wait for approval", hint: `${c.approvals.filter((a) => a.decision === "approved").length} of ${c.approvals.length} approved.` })
        ...
        <RecordHeader
          ...
          actions={
            <>
              ...
              {c.status === "awaiting_approval" && myApproval?.decision === "pending" && (
                <>
                  <form action={decideApproval.bind(null, c.id, "approved", undefined)}><Button type="submit" size="sm" variant="primary"><Check className="size-3.5" /> Approve</Button></form>
                  <form action={decideApproval.bind(null, c.id, "rejected", undefined)}><Button type="submit" size="sm" variant="danger"><X className="size-3.5" /> Reject</Button></form>
                </>
              )}
              ...
              {c.status === "approved" && <form action={transitionChange.bind(null, c.id, "in_progress")}><Button type="submit" size="sm" variant="primary">Start change</Button></form>}
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_320px] overflow-y-auto">
          <div className="space-y-4 px-6 py-4">
            <ol className="grid grid-cols-6 gap-1.5">
              {STEPS.map((s, i) => (
                <li key={s}>
                  ...
                  <p className={`mt-1 text-[11px] ${i <= stepIdx ? "text-ink" : "text-ink-4"}`}>{c.status === "rolled_back" && s === "completed" ? "Rolled back" : CHANGE_STATUS_LABEL[s]}</p>
                </li>
              ))}
            </ol>
            ...
            <Block title="Tasks" action={<span className="text-[12px] text-ink-3">{openTasks} open</span>}>
              <TaskList rows={tasks} parentType="change" parentId={c.id} back={back} />
            </Block>
          </div>
          <aside className="space-y-4 bg-surface px-4 py-4 hairline-l">
            <div>
              <p className="label mb-2">Approvals</p>
              {c.approvals.length === 0 ? (
                ...
                <ul className="space-y-1.5">
                  {c.approvals.map((a) => (
                    <li key={a.personId} className="flex items-center gap-2 text-[13px]">
                      <Avatar name={a.name} size={20} />
                      <span className="flex-1 truncate">{a.name}</span>
                      <Tone tone={a.decision === "approved" ? "ok" : a.decision === "rejected" ? "crit" : "warn"} className="capitalize">{a.decision}</Tone>
                    </li>
                  ))}
                </ul>
              )}
