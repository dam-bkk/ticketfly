  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="mx-auto max-w-[1100px] space-y-3 px-6 py-4 rise">
          <div className="flex items-end justify-between">
            <div>
              <h1 className="text-[22px] font-semibold tracking-[-0.01em]">SLA performance</h1>
              <p className="text-[13.5px] text-ink-3">Last 90 days, business hours, paused time excluded. Target 95%.</p>
            </div>
                ...
                {byPriority.map((p) => {
                  ...
                  return (
                    <tr key={p.priority} className="hairline-t">
                      <td className="px-4 py-2"><PriorityMark priority={p.priority} withLabel /></td>
                      <td className="tnum px-4 py-2 text-right">{p.resolved}</td>
                      <td className="tnum px-4 py-2 text-right">{p.met}</td>
                      ...
                      <td className={cn("tnum px-4 py-2 text-right", fr >= 0.95 ? "text-ok" : fr >= 0.8 ? "text-warn" : "text-crit")}>{pct(fr)}</td>
                      <td className={cn("tnum px-4 py-2 text-right", p.openBreached && "font-medium text-crit")}>{p.openBreached}</td>
                      <td className="tnum px-4 py-2 text-right text-ink-3">{p.paused}</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          <section className="panel p-4">
            <div className="mb-3 flex items-baseline justify-between">
              <h2 className="text-[13.5px] font-semibold">Weekly attainment</h2>
              <span className="flex gap-3 text-[12.5px] text-ink-3"><span className="flex items-center gap-1.5"><i className="size-2 rounded-sm bg-ink-4" /> Resolved</span><span className="flex items-center gap-1.5"><i className="size-2 rounded-sm bg-ok" /> Within SLA</span></span>
            </div>
          ...
          <section className="panel overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2 hairline-b">
              <h2 className="text-[13.5px] font-semibold">Open tickets past resolution target</h2>
              <Link href="/tickets?f=at_risk" className="text-[12.5px] font-medium text-accent-ink hover:underline">Open the queue</Link>
            </div>
            {breaches.length === 0 ? <Empty title="Nothing past its resolution target" hint="Every open ticket is inside its SLA. Tickets due in the next 24 hours are listed under At risk." action={<ButtonLink href="/tickets?f=at_risk" size="sm">Open the at-risk queue</ButtonLink>} /> : (
              <ul className="divide-y divide-line">
                {breaches.map((b) => (
                  <li key={b.id} className="row flex items-center gap-3 px-4 py-2 text-[13.5px]">
                    ...
                    <span className="text-[12.5px] text-ink-3">{b.group}</span>
                    {b.assignee ? <span className="flex items-center gap-1.5 text-[12.5px] text-ink-3"><Avatar name={b.assignee} size={16} /> {b.assignee}</span> : <span className="text-[12.5px] text-warn">Unassigned</span>}
          ...
          <p className="text-[12.5px] text-ink-3">{PRIORITY_LABEL.urgent} runs on calendar hours (24×7); the others on Hong Kong business hours. Edit in Admin → SLA and OLA policies.</p>
        </div>
      </div>
    </>
  );
