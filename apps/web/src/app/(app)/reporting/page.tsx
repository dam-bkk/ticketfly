...
export default async function ReportingPage() {
  ...
  const colors: Record<string, string> = { portal: "var(--accent)", email: "var(--info)", agent: "var(--ok)", import: "var(--ink-4)", system: "var(--warn)" };
  return (
    <>
      <Topbar crumbs={[{ label: "Reporting" }, { label: "Analytics" }]} />
      <div className="flex-1 overflow-y-auto">
        <div className="mx-auto max-w-[1180px] space-y-3 px-6 py-4 rise">
          <div className="flex items-end justify-between">
            <div>
              <h1 className="text-[22px] font-semibold tracking-[-0.01em]">Analytics</h1>
              <p className="text-[13.5px] text-ink-3">Every number is a query on our own database — no report tiers, no export caps. Last 12 months unless stated.</p>
            </div>
            <div className="flex gap-2">
              <ButtonLink href="/reporting/sla" size="sm">SLA performance</ButtonLink>
              <ButtonLink href="/api/activity.csv" size="sm">Export activity CSV</ButtonLink>
            </div>
          </div>

          <section className="panel p-4">
            <div className="mb-3 flex items-baseline justify-between">
              <h2 className="text-[13.5px] font-semibold">Volume by month</h2>
              <span className="flex gap-3 text-[12.5px] text-ink-3"><span className="flex items-center gap-1.5"><i className="size-2 rounded-sm bg-accent" /> Created</span><span className="flex items-center gap-1.5"><i className="size-2 rounded-sm bg-ok" /> Resolved</span></span>
            </div>
            ...
            <div className="mt-3 grid grid-cols-12 gap-1 text-center text-[11px] text-ink-3">
              {monthly.map((m) => (
                <span key={m.label} className="tnum">{m.median_hours ? `${Math.round(m.median_hours)}h` : "—"}</span>
              ))}
            </div>
            <p className="mt-1 text-center text-[11px] text-ink-3">Median time to resolve, per month</p>
          </section>

          <div className="grid gap-3 lg:grid-cols-2">
            <section className="panel overflow-hidden">
              <div className="px-4 py-2 hairline-b"><h2 className="text-[13.5px] font-semibold">By team</h2></div>
            ...
            <section className="panel overflow-hidden">
              <div className="px-4 py-2 hairline-b"><h2 className="text-[13.5px] font-semibold">By category · 90d</h2></div>
              <table className="w-full text-[13.5px]">
          ...
          <div className="grid gap-3 lg:grid-cols-3">
            <section className="panel p-4">
              <h2 className="mb-3 text-[13.5px] font-semibold">Time to resolve</h2>
              <ul className="space-y-2">
                {buckets.map((b) => (
                  <li key={b.label} className="grid grid-cols-[56px_1fr_36px] items-center gap-2 text-[12.5px]">
            ...
            <section className="panel p-4">
              <h2 className="mb-3 text-[13.5px] font-semibold">Requests by department · 90d</h2>
              {byDept.length === 0 && <Empty title="No requests in the last 90 days" action={<ButtonLink href="/tickets" size="sm">Open the ticket list</ButtonLink>} />}
              <ul className="space-y-2">
                {byDept.map((d) => (
                  <li key={d.name ?? "none"} className="grid grid-cols-[120px_1fr_36px] items-center gap-2 text-[12.5px]">
                    <span className="truncate text-ink-2">{d.name ?? "—"}</span>
                    <span className="bar"><i style={{ width: `${(Number(d.n) / maxDept) * 100}%` }} /></span>
            ...
            <section className="panel p-4">
              <h2 className="mb-3 text-[13.5px] font-semibold">Channel and satisfaction</h2>
              ...
              <div className="mt-4">
                ...
                <p className="mt-1.5 text-[11px] text-ink-3">{csatTotal} ratings · {pct(csat.filter((c) => Number(c.score) >= 4).reduce((s, c) => s + Number(c.n), 0) / csatTotal)} rated 4 or 5</p>
              </div>
            </section>
          </div>
        </div>
