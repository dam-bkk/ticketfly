  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="mx-auto max-w-[760px] px-6 py-6 rise">
          <h1 className="text-[20px] font-semibold tracking-[-0.01em]">Notification preferences</h1>
          <p className="text-[13px] text-ink-3">Same list, three channels. Email uses the support mailbox; Teams arrives with the Servicebot phase.</p>
          <form action={saveNotifyPrefs} className="panel mt-5 overflow-hidden">
                ...
                {KINDS.map(([k, label, desc]) => {
                  ...
                  return (
                    <tr key={k} className="hairline-t">
                      <td className="px-4 py-2.5"><span className="block font-medium">{label}</span><span className="text-[12px] text-ink-3">{desc}</span></td>
                      {(["inApp", "email", "teams"] as const).map((ch) => (
                        <td key={ch} className="px-4 py-2.5 text-center"><input type="checkbox" name={`${k}.${ch}`} defaultChecked={v[ch]} className="size-3.5 accent-[var(--accent)]" /></td>
                      ))}
                    </tr>
                  );
                })}
            ...
            <div className="flex justify-end px-4 py-3 hairline-t"><Button type="submit" variant="primary" size="sm">Save</Button></div>
          </form>
        </div>
      </div>
    </>
